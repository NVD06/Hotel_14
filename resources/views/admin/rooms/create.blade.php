...
<div class="max-w-3xl mx-auto p-6">
    ...
    <form method="POST" action="{{ route('admin.rooms.store') }}" class="space-y-5" enctype="multipart/form-data">
        ...
        <div>
            ...
            @error('images.*') <p class="text-red-600 text-sm">{{ $message }}</p> @enderror
            <p class="text-xs text-gray-500 mt-1">Cho phép: JPG, PNG, WEBP (tối đa 2MB/ảnh)</p>
        </div>
        ...
        <script>
            document.querySelector('input[name="images[]"]').addEventListener('change', e => {
                const wrap = document.getElementById('previews') || document.body.appendChild(Object.assign(document.createElement('div'), {
                    id: 'previews',
